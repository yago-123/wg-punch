...
func main() {
	...
	conn := connect.NewConnector("p22", puncher, tunnel, rendezvous, 1*time.Second)
	...
	netConn, err := conn.Connect(ctx, localAddr, []string{WireGuardIfaceAddrCIDR}, "p11", localPrivKey, localPubKey)
	if err != nil {
